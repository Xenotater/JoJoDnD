    ...
    <Modal fullPage blur closeCallback={closeCallback}>
      <div className="content shadow-lg/80 h-[75%] w-[80%] flex flex-col">
        <div className="relative flex">
          <Input type="search" placeholder={t("search")} className="bg-white border-1 px-2 text-lg" value={manager.settings.search} onBlur={(e) => manager.updateSetting("search", e.target.value)} />
        ...
        <div className="h-[calc(100%-102px)] overflow-y-scroll">
          ...
          {loading ? (
            ...
            <div className="flex flex-wrap gap-4 2xl:gap-12 justify-center pb-6">
              ...
              <div className="basis-full md:basis-1/3 lg:basis-1/4 xl:basis-1/5 flex justify-center">
                <div className="relative w-[204px] h-[250px] border-2 rounded-sm flex justify-center items-center hover:shadow-lg/66 cursor-pointer" onClick={() => setNewOpen(true)}>
                  {newOpen && 
                    <NewItemMenu closeCallback={(type?: "folder" | "character") => {
                      setNewOpen(false);
                      if (type == "folder")
                        updateStates();
