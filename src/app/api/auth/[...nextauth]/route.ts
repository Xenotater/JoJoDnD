  ...
  providers: [
    CredentialsProvider({
      ...
      authorize: async function (credentials: Record<"username" | "password", string> | undefined, req: Pick<RequestInternal, "body" | "query" | "headers" | "method">): Promise<User | null> {
        const resp = await doDBQuery("SELECT id, username, email, password FROM users WHERE username = ? LIMIT 1", [credentials?.username ?? ""], false);
        if (resp.status === 200) {
          const data = (await resp.json())[0] as {id: number, username: string, email: string, password: string};
          if (data.id) {
            ...
            if (hash === createHash("sha512").update(credentials?.password + salt).digest("hex"))
              return {id: data.id.toString(), name: data.username, email: data.email};
          }
        }
        return null;
      }
