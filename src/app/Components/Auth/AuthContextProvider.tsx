...
export default function AuthContextProvider ({children}: {children: React.ReactNode}) {
  ...
  return (
    <AuthContext value={{
      ...
      authExecute: (callback: () => void, onSuccess = true) => {
        if (session?.user)
          callback();
        else {
          if (onSuccess)
      ...
      <>
        {authAction != "None" && 
          <AuthModalWrapper initialAction={authAction} closeCallback={() => {setAuthAction("None"); setSuccessCallback(() => {})}} successCallback={successCallback}/>
        }
        {children}
      </>
    </AuthContext>
