  ...
  return (
    <>
      <div data-key={toTitleCase(content.name.replace("'", ""))} data-nav={content.isLink ?? true} onClick={() => handleClick()} onKeyDown={(e) => handleKeyDown(e)} tabIndex={0}
        className={`relative flex outline-none ${isLink || content.onClick ? "cursor-pointer" : ""} ${isSelected ? "font-bold bg-jj-mpurple-2 hover:bg-jj-mpurple-3 focus:bg-jj-mpurple-3" : ""}
          ${isLink || hasSubItems || content.onClick ? "hover:bg-jj-mpurple-2 focus:bg-jj-mpurple-2" : ""} ${hasSubItems ? "pr-6.5" : ""}`}
      >
